		...
		if (resultObject.getCode() == 0) {
			if (isVote == 1) {// 未
				...
				if (voteResult == 1) {
					...
					aq.id(R.id.btn_radio2).clickable(false);
					aq.id(R.id.btn_radio3).clickable(false);
					aq.id(R.id.radio_asscess).clickable(false);
				} else if (voteResult == 2) {
					aq.id(R.id.btn_radio1).clickable(false);
