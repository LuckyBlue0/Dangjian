...
import cn.com.do1.component.post.postinfo.service.impl.PostinfoServiceImpl;
...
//				loginSuccess(userLoginvo);
	...
	public void loginQnh() throws Exception, BaseException {
		HttpServletRequest request = ServletActionContext.getRequest();
		String username = request.getParameter("username");
		String userPwd = request.getParameter("userPwd");		
		
		if (AssertUtil.isEmpty(username)) {
			addJsonObj("desc", "username参数不能为空");	
			addJsonObj("status", "1");	
		}
		if (AssertUtil.isEmpty(userPwd)) {
			addJsonObj("desc", "userPwd参数不能为空");	
			addJsonObj("status", "1");	
		}
		...
		Md5 m = new Md5();		
		String userPwdMd5 = m.getMD5ofStr(userPwd.trim()).toLowerCase();
		LoginUserVO user = new LoginUserVO();
		user.setUsername(username);
		user.setUserPwd(userPwdMd5);
		LoginUserVO userLoginvo = indexService.loginUser(user, userType);
		
		if (null != userLoginvo) {
			addJsonObj("userId", userLoginvo.getUserId());
			addJsonObj("username", userLoginvo.getUsername());
			addJsonObj("name", userLoginvo.getName());
			addJsonObj("userType", userLoginvo.getUserType());	
			addJsonObj("status", "0");		
		} else {
			addJsonObj("desc", "用户名或密码错误");	
			addJsonObj("status", "1");	
		}
	 ...
	 */
	@JSONOut(catchException = @CatchException(errCode = "1001", successMsg = "操作成功", faileMsg = "操作失败"))
	public void myUserInfoQnh() throws BaseException, Exception{
		...
		try{
			...
			if (AssertUtil.isEmpty(userId)) {
				addJsonObj("desc", "userId参数不能为空");	
				addJsonObj("status", "1");	
			}
			
			getSearchValue().put("userId", userId);
			PartyMenberUserInfoVO  partyMenberInfo =  userInfoService.countUserInfo(getSearchValue());
			
			List partyMenberList = new ArrayList();
			partyMenberList.add(partyMenberInfo);
			
			if(!AssertUtil.isEmpty(partyMenberInfo)){
				addJsonObj("list", partyMenberList);						
				addJsonObj("status", "0");		
			}else{
				addJsonObj("status", "1");		
			}
