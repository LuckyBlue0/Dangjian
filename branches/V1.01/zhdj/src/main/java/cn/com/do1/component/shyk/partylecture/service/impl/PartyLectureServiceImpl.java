package cn.com.do1.component.shyk.partylecture.service.impl;
...
* User: ${user}
*/
@Service("partyLectureService")
public class PartyLectureServiceImpl extends BaseService implements IPartyLectureService {
    private final static transient Logger logger = LoggerFactory.getLogger(PartyLectureServiceImpl.class);

    private IPartyLectureDAO partyLectureDAO;
    @Resource
    private IOrgService orgService;
    @Resource
    private IMeetinguserService meetinguserService;
    @Resource
    public void setPartyLectureDAO(IPartyLectureDAO partyLectureDAO) {
        this.partyLectureDAO = partyLectureDAO;
        setDAO(partyLectureDAO);
    }

    public Pager searchPartyLecture(Map searchMap,Pager pager) throws Exception, BaseException {
    	return partyLectureDAO.pageSearchByField(searchMap,pager);
    }

	@Override
	public Map<String, Object> savePartyLectureInfo(TbPartyLecturePO tbPartyLecturePO, String userIds) throws Exception, BaseException {
		...
		if(tbPartyLecturePO != null){
			...
			if(!AssertUtil.isEmpty(orgVO) && !AssertUtil.isEmpty(orgVO.getOrganizationId())){
				tbPartyLecturePO.setOrganizationId(orgVO.getOrganizationId());
			}else if(user.getUsername().equals("admin")){
				logger.info("系统管理员没有归属组织,不需要记录组织");
			}
			if(tbPartyLecturePO.getSmsNotice() == null){
				tbPartyLecturePO.setSmsNotice(0l);
			}
			if(AssertUtil.isEmpty(tbPartyLecturePO.getId())){
				String meetingId = UUID.randomUUID().toString();
				tbPartyLecturePO.setId(meetingId);
				tbPartyLecturePO.setCreateTime(new Date());
				tbPartyLecturePO.setWhetherEnd(0l);
				tbPartyLecturePO.setWhetherRecommend(0l);
				tbPartyLecturePO.setStatus(0l);
				tbPartyLecturePO.setCreateUserId(user.getPersonId());
				tbPartyLecturePO.setCreateUserName(user.getPersonName());
				// 生成二维码,返回二维码存放路径
				StringBuffer params = new StringBuffer();
				params.append("title:").append(tbPartyLecturePO.getTitle()).append(";").append("id:").append(tbPartyLecturePO.getId());
				String imgPath = GenerateQRCode.getInstance().generateRrCode(
						tbPartyLecturePO.getId(), params.toString(),null);
				//生成二维码
				tbPartyLecturePO.setQrCode(imgPath);
				this.partyLectureDAO.insertData(tbPartyLecturePO);
				if(!AssertUtil.isEmpty(userIds)){
					...
					for(String id : userId){
						...
						meetingUser.setActivityType(2l);//会议类型
						this.partyLectureDAO.insert(meetingUser);
						addUserCount++;
					}
				...
				this.partyLectureDAO.update(tbPartyLecturePO, false);
				Map userMap = getMeetingUserMap(tbPartyLecturePO.getId());
				String [] uids = null;
				int addCount = 0;
				if(!AssertUtil.isEmpty(userIds)){
					uids = userIds.split(",");
					...
					for(String id : uids){
						...
						if(!userMap.containsKey(id)){
							...
							meetingUser.setId(UUID.randomUUID().toString());
							meetingUser.setUserId(id);
							meetingUser.setMeetingId(tbPartyLecturePO.getId());
							meetingUser.setCreateTime(new Date());
							meetingUser.setSignUpStatus(0l);
							meetingUser.setForLeaveStatus(0l);
							meetingUser.setSignInStatus(0l);
				...
				while (it.hasNext()) {
					...
					this.meetinguserService.deleteMeetingByMeetingIdAndUserId(tbPartyLecturePO.getId(),entry.getKey());
					delCount++;
				}
	...
	@Override
	public Map<String, Object> savePartyLectureRecordInfo(TbPartyLecturePO tbPartyLecturePO, String userIds) throws Exception, BaseException {
		// TODO Auto-generated method stub
		Map<String,Object> result = new HashMap<String,Object>();
		String [] uids = null;
		int addCount = 0;
		this.partyLectureDAO.update(tbPartyLecturePO, false);
		Map userMap = getSupplementUserMap(tbPartyLecturePO.getId());
		if(!AssertUtil.isEmpty(userIds)){
			uids = userIds.split(",");
			TbMeetingUserPO meetingUser = null;
	...
	@Override
	public PartyLectureVO searchById(String id) throws Exception, BaseException {
		return partyLectureDAO.searchById(id);
	}
}
