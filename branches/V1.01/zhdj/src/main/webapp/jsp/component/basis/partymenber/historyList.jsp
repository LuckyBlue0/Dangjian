			...
			<form
				...
				<div class="searchWrap">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td class="searchLeft">
								<input name="searchValue.userType" value="3" type="hidden" />
							</td>
							<td class="searchBg">
											 ...
											 -->
													<td align="right">
														...
														<input class="btnQuery" type="button" value="新增"
															onclick="javascript:window.location.href='add.jsp'+ '?userType=3&dqdp_csrf_token='+dqdp_csrf_token" />
														<input class="btnQuery" type="button" value="批量导入"
															onclick="javascript:window.location.href='import.jsp'+ '?userType=3&dqdp_csrf_token='+dqdp_csrf_token" />
...
function _doSignlChange(formId, content) {
	$.ajax({
				url : '${baseURL}/partymenber/partymenberAction!ajaxChangeState.action?tbPartyMenberInfoPO.id='
						+ content.id + '&tbPartyMenberInfoPO.state=1&tbPartyMenberInfoPO.userType=1',
				type : 'post',
				dataType : 'json',
				success : function(result) {
					if ('0' == result.code) {
