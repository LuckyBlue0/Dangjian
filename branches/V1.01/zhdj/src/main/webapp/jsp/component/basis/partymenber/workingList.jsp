										...
										<td>
											<table class="search" width="100%" border="0" cellspacing="0"
												...
												<td width="50">
													...
													<td width="200">
														<input class="for50px" id="parentName"
															style="width: 150px" type="text" size="10"
															readonly="readonly"
													...
													<td align="right">
														...
														<input class="btnQuery" type="button" value="新增"
															onclick="javascript:window.location.href='add.jsp'+ '?userType=1&dqdp_csrf_token='+dqdp_csrf_token" />
														<input class="btnQuery" type="button" value="批量导入"
															onclick="javascript:window.location.href='import.jsp'+ '?userType=1&dqdp_csrf_token='+dqdp_csrf_token" />
...
function _doSignlChange(formId, content, state) {
	$.ajax({
				url : '${baseURL}/partymenber/partymenberAction!ajaxChangeState.action?tbPartyMenberInfoPO.id='
						+ content.id + '&tbPartyMenberInfoPO.state=' + state,
				type : 'post',
				dataType : 'json',
				success : function(result) {
					if ('0' == result.code) {
