...
<form action="${baseURL}/partyfile/partyfileAction!ajaxAdd.action" method="post" id="org_add" enctype="multipart/form-data">
    <input type="hidden" name="tbPartyFilePO.fileSize" id="fileSize"/>
     <div class="searchWrap">
            <div class="title">
                <h2 class="icon1">新增</h2>
            </div>
        </div>
	  ...
	  // $('#org_add').attr("action","${baseURL}/videofile/videofileAction!uploadFile.action?dir=videoFile"); 
	   ...
	   if(""==$("#file").val()){
        ...
        if ($.inArray(fileext,allowtype) == -1)
          {
            alert("文件上传只支持'ppt','doc','xls','txt' 类型");  
            return;
          }
	   ...
	   var myDate = new Date();
		...
		$("#progress").show();
	     ...
	     $('#org_add').attr("action","${baseURL}/partyfile/partyfileAction!ajaxAdd.action"); 
   }
</script>
