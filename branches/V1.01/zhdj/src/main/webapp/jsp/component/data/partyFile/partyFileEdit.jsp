...
<jsp:include page="../../../common/dqdp_vars.jsp">
    ...
    <jsp:param name="dict" value="partyType"></jsp:param>
</jsp:include>
<head>
...
<form action="${baseURL}/partyfile/partyfileAction!ajaxUpdate.action" method="post" id="org_add" enctype="multipart/form-data">
   <div class="searchWrap">
    ...
    <table class="tableCommon mt5" width="100%" border="0" cellspacing="0" cellpadding="0" datasrc="${baseURL}/partyfile/partyfileAction!ajaxView.action?id=${param.id}">
       <input type="hidden" name="tbPartyFilePO.id"/>
        <input type="hidden" name="tbPartyFilePO.fileSize" id="fileSize"/>

        <tbody>
            <tr>
                <td class="tdBlue"><span><font color="red">*</font></span>文件类型</td>
                <td >
                    <select class="form120px"  name="tbPartyFilePO.type" dictType="partyType"
            ...
            <tr>
                ...
                <td >
                    <input class="form120px"  name="tbPartyFilePO.fileName" id="fileName"  type="text" 
            ...
            <tr>
                ...
                <td >
                   ...
                   <input name="tbPartyFilePO.filePath" id="imgPath" class="form120px"  type="hidden"   />&nbsp; 
            ...
            <tr>
                ...
                <td >
                <textarea rows="3" name="tbPartyFilePO.remark"  valid="{must:false,tip:'文件说明',length:2000}" ></textarea>
                </td>
            </tr>
        ...
        if (dqdp.submitCheck("org_add")) {
                    ...
                    success:function (result) {
                        if ("0" == result.code) {
                            alert('修改成功'); 
                            if(""!=$("#file").val()){
                                 window.location.href=("partyFileList.jsp?filePath="+$("#imgPath").val()+"&fileName="+$("#fileName").val()); 
                            }else{
                            	 window.location.href=("partyFileList.jsp");
                            }
                        } else {
                            alert(result.desc);
                        }
   ...
   function uploadFile(){
	  // $('#org_add').attr("action","${baseURL}/videofile/videofileAction!uploadFile.action?dir=videoFile"); 
	   ...
	   if(""==$("#file").val()){
        ...
        var allowtype =  ["PPT","DOC","XLS","TXT"];
        if ($.inArray(fileext,allowtype) == -1)
          {
            alert("视频文件上传只支持'ppt','doc','xls' 类型");  
            return;
          }
	   $('#org_add').attr("action","${baseURL}/progressUpload?dir=partyFile"); 
	   var myDate = new Date();
		...
		$("#progress").show();
	     ...
	     $('#org_add').attr("action","${baseURL}/partyfile/partyfileAction!ajaxUpdate.action"); 
   }
</script>
