...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<jsp:include page="../../../common/dqdp_vars.jsp">
		...
		<jsp:param name="permission"
			value="partyFileManage,partyFileAdd,partyFileEdit,partyFileDel"></jsp:param>
	...
	<body style="background: #f6ebe5;">
		...
		<!-- 新闻类型信息搜索表单 -->
		<form action="${baseURL}/partyfile/partyfileAction!ajaxSearch.action"
									...
									<td>
										<table class="search" width="100%" border="0" cellspacing="0"
											...
											<tr>
												...
												<td width="140">
													<select  name="searchValue.type" dictType="partyType"></select>
												</td>
												<td>
													<input class="btnQuery" name="search" type="button"
														value="查询" permission=""
														onclick="javascript:doSearch(1);" />
											        <input class="btnQuery" type="button" value="新增" permission="partyFileAdd" onclick="javascript:doVideoFileAdd()"/>&nbsp;
	                                                <input class="btnQuery" type="button" value="删除" permission="partyFileDel" onclick="javascript:_doDel('userInfo_list');doSearch(1);" />
												</td>
											</tr>
			...
			delUrl="${baseURL}/partyfile/partyfileAction!ajaxBatchDelete.action"></div>
		<div id="tip"></div>
...
$(document).ready(function() {
	doSearch(1);
	 ...
	 $.ajax({
            url:'${baseURL}/partyfile/partyfileAction!uploadFile.action',
            data:{"tbPartyFilePO.filePath":$("#filePath").val(),"tbPartyFilePO.fileName":$("#fileName").val()}, 
            type:'post',                                     
            dataType:'json',
											...
											operations : [
													{
														...
														event : function(index,
																content) {
															window.location.href = 'partyFileView.jsp?id='+ content.id;
														}
													},
													{
														name : "修改",
														permission : "partyFileEdit",
														event : function(index, 
																content) {
															window.location.href = 'partyFileEdit.jsp?id='+ content.id;
																
														}
													},
													{
														name : "删除",
														permission : "partyFileDel",
														event : function(index,
																content) {
															_doSignlDel(
																	"userInfo_list",
...
function doVideoFileAdd(){
	window.location.href="${baseURL}/jsp/component/data/partyFile/partyFileAdd.jsp"; 
}
</script>
