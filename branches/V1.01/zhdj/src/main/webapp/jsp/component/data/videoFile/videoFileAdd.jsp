...
<form action="${baseURL}/videofile/videofileAction!ajaxAdd.action" method="post" id="org_add" enctype="multipart/form-data">
    ...
    <table class="tableCommon mt5" width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tbody>
            <tr>
                <td class="tdBlue"><span><font color="red">*</font></span>视频类型</td>
                <td>
            ...
            <tr>
                ...
                <td>
                   ...
                   <input name="tbVideoPO.filePath" id="imgPath" class="form120px"  type="hidden"   />&nbsp; 
        ...
        if (dqdp.submitCheck("org_add")) {
                    ...
                    success:function (result) {
                        if ("0" == result.code) {
                            ...
                            window.location.href=("videoFileList.jsp?filePath="+$("#imgPath").val()+"&fileName="+$("#fileName").val()); 
                        } else {
                            alert(result.desc);
                        }
                    },   
   ...
   function uploadFile(){
	  // $('#org_add').attr("action","${baseURL}/videofile/videofileAction!uploadFile.action?dir=videoFile"); 
	   ...
	   if(""==$("#file").val()){
        ...
        var allowtype =  ["RM","RMVB","3GP","AVI","swf","flv"];
        if ($.inArray(fileext,allowtype) == -1)
          {
            alert("视频文件上传只支持'RM','RMVB','3GP','AVI','swf','flv' 类型");  
            return;
          }
	   $('#org_add').attr("action","${baseURL}/progressUpload?dir=videoFile"); 
	   var myDate = new Date();
		...
		$("#progress").show();
	     ...
	     $('#org_add').attr("action","${baseURL}/videofile/videofileAction!ajaxAdd.action"); 
   }
</script>
