        ...
        $('#role_search').ajaxSubmit({
            ...
            success:function (result) {
                if ("0" == result.code) {
                    ...
                    // 将用户已有的角色选中
                    var roleIdArray = window.opener.getSelectRoleIdArray();
    ...
    function initCheckEvent() {
        $('[name=ids]').click(function () {
            if ($(this).prop('checked')) {
                window.opener.addRole($(this).val(), $(this).parent().parent().find('td').eq(1).html());
            } else {
                window.opener.removeRole($(this).val(), $(this).parent().parent().find('td').eq(1).html());
            }
        });
        // 添加全选checkbox事件
