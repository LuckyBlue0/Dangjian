...
<form action="${baseURL}/orgvote/orgvoteAction!ajaxSave.action" method="post" id="id_form_search">
    ...
    <table class="tableCommon mt5" width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
            ...
            <td >
            	<input type="hidden" id="orgVoteId" value="${tbOrgVotePO.id}" name="tbOrgVotePO.id"/>
            	<input type="hidden" id="pushStatus" value="${tbOrgVotePO.pushStatus}" name="tbOrgVotePO.pushStatus"/>
                <input type="text" id="topic" value="${tbOrgVotePO.topic}" name="tbOrgVotePO.topic" valid="{must:true,tip:'所属专题',fieldType:'pattern'}"/>&nbsp;&nbsp;&nbsp;
            </td>
        </tr>
        <tr>
            <td class="tdBlue"  ><span style="color: red">*</span>评议时间</td>
             <td>开始时间
				<input
					...
					value="<fmt:formatDate value="${tbOrgVotePO.startTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
                   &nbsp;&nbsp;&nbsp;&nbsp;结束时间
				<input
					...
					value="<fmt:formatDate value="${tbOrgVotePO.endTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
            </td>
        </tr>
        <tr>
            <td class="tdBlue"><span style="color: red">*</span>备注</td>
            <td>
                 <textarea id="topic"  name="tbOrgVotePO.remark" rows="5" cols="50">${tbOrgVotePO.remark}</textarea>
            </td>
        </tr>
        </tbody>
    </table>
    <table id="orgList" class="tableCommon mt5" width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<tr>
			<td class="tdBlue">
				<input type="hidden" name="tbVoteOrgList[${index.index}].orgVoteId" value="${id}"  />
	    		<input type="hidden" name="tbVoteOrgList[${index.index}].id" value="${voteOrg.id}"  />
			</td>
			<td>
				<input type="text" id="voteOrgName${index.index}" value="${voteOrg.voteOrg}" name="tbVoteOrgList[${index.index}].voteOrg" valid="{must:true,tip:'被评选机关',fieldType:'pattern'}"/>
