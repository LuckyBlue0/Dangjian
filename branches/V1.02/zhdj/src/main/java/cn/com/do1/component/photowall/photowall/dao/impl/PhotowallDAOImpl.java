...
public class PhotowallDAOImpl extends BaseDAOImpl implements IPhotowallDAO {
    ...
    public Pager searchTbPhotoWall(Map searchValue, @Security(encode = "")Pager pager) throws Exception, BaseException {
    	...
    	if(AssertUtil.isEmpty(mobileClient)){
    		...
    		sb.append("SELECT t.*,m.name user_name FROM TB_PHOTO_WALL t left join TB_PARTY_MENBER_INFO m on m.id=t.CREATE_USER_ID")
    		.append(" where t.type=:type and t.status=0");
    		
    		if(!AssertUtil.isEmpty(searchValue.get("keyword"))){
    			sb.append(" and ( t.title like :keyword or t.des like :keyword )");
    		}
    		sb.append(" order by t.CREATE_TIME desc");
    		
    		String searchSQL = sb.toString(); 
    	    String countSQL = "select count(1) from (" + searchSQL+"  ) a ";
    		pager = super.pageSearchByField(PhotoWallListResponse.class, countSQL, searchSQL, searchValue, pager);
    		if(!AssertUtil.isEmpty(pager) && !AssertUtil.isEmpty(pager.getPageData())){
    			for(PhotoWallListResponse rep : (List<PhotoWallListResponse>)pager.getPageData()){
    				if(!AssertUtil.isEmpty(rep)){
    					List<TbPhotoWallImgVO> list = findPhotowallImgsById(rep.getId());
    					if(!AssertUtil.isEmpty(list)){
    						rep.setImgUrl(list.get(0).getImgUrl());
    						rep.setThumImgUrl(list.get(0).getThumImgUrl());
    					}
    				}
    			}
    		}
	...
	public PhotoWallDetailsResponse findPhotoWallById(String id) throws SQLException {
		...
		StringBuffer sb = new StringBuffer();
		sb.append("SELECT t.title,t.des,t.create_time as push_time,m.name as author,t3.review_count From TB_PHOTO_WALL t left join TB_PARTY_MENBER_INFO m on m.id=t.CREATE_USER_ID")
