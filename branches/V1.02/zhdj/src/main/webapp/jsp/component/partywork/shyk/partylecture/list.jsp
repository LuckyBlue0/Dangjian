...
<jsp:include page="../../../../common/dqdp_vars.jsp">
    ...
    <jsp:param name="permission" value="partyLectureManager,partyLectureSearch,partyLectureView,partyLectureConclusion,partyLectureAdd,partyLectureEdit,partyLectureDel,partyLectureRecommended,partyLectureRecord,partyLecturePush,partyLectureOut"></jsp:param>
...
<form action="${baseURL}/partyLecture/partyLectureAction!ajaxSearch.action" method="post" id="form_search">
...
<!-- 新闻类型信息搜索表单 -->
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                        	<input class="btnQuery" name="search" type="button" value="查询" permission="" onclick="javascript:doSearch(1);"/>
       										<input class="btnQuery" type="button" permission="partyLectureAdd" value="新增" onclick="javascript:window.location.href='add.jsp'+ '?dqdp_csrf_token='+dqdp_csrf_token" />
       										<input class="btnQuery" type="button" permission="partyLectureDel" value="删除" onclick="javascript:_doDel('partyLectureInfo_list');doSearch(1);"/>
										</td>
                                    </tr>
	...
	<div id="partyLectureInfo_list" delUrl="${baseURL}/partyLecture/partyLectureAction!ajaxBatchDelete.action"></div>
	<div id="tip"></div>
	<div class="toolBar">
...
<script type="text/javascript">
                    ...
                    var list1 = new ListTable(
                            {
                                ...
                                title:[
                                    ...
                                    {width:"8%", name:"typeDesc", showName:"会议类型"},
                                ...
                                operations:[
                                    {name:"查看", permission:"partyLectureView", event:function (index, content) {
                                        window.location.href = 'view.jsp?id=' + content.id+ '&dqdp_csrf_token='+dqdp_csrf_token;
                                    }},
                                    {name:"二维码", permission:"partyLectureView", event:function (index, content) {
                                        window.location.href = '${baseURL}/jsp/component/partywork/shyk/meeting/recode.jsp?id=' + content.id+ '&type=2';
                                    }},
                                    {name:"会议记录", permission:"partyLectureRecord", event:function (index, content) {
                                         window.location.href="record.jsp?id="+ content.id+ '&dqdp_csrf_token='+dqdp_csrf_token;
                                    }},
                                    {name:"总结发布", permission:"partyLectureConclusion", condition:function (index, content) {
											return content.whetherEnd == 1;
											return true;
											},event:function (index, content) {
                                        window.location.href = '${baseURL}/jsp/component/news/tissuemien/edit.jsp?id=' + content.id+ '&type=1&backURL=${baseURL}/jsp/component/partywork/shyk/partylecture/list.jsp&dqdp_csrf_token='+dqdp_csrf_token;
                                    }},
                                    {name:"编辑", permission:"partyLectureEdit", condition:function (index, content) {
											return content.status == 0;
											}, event:function (index, content) {
                                    	window.location.href = 'edit.jsp?id=' + content.id+ '&dqdp_csrf_token='+dqdp_csrf_token;
                                    }}, 
                                    {name:"发布", permission:"partyLecturePush", condition:function (index, content) {
											return content.status == 0;
											}, event:function (index, content) {
                                    	pushOrOut(content.id,1);
                                    }}, 
                                    {name:"下架", permission:"partyLectureOut", condition:function (index, content) {
											return content.status == 1;
											}, event:function (index, content) {
                                    	pushOrOut(content.id,2);
                                    }}, 
	                                {name:"删除", permission:"partyLectureDel", condition:function (index, content) {
											return content.status != 1;
											},event:function (index, content) {
	                                    _doSignlDel("partyLectureInfo_list", content.id);
	                                }}
                                    
                                ],
                                trStyle:["trWhite"]
                            });
                    list1.createList("partyLectureInfo_list");
 ...
 function pushOrOut(id,type){
	...
	if(confirm(alertDesc)){
	    $.ajax({
	        url: "${baseURL}/partyLecture/partyLectureAction!pushOrOut.action",
