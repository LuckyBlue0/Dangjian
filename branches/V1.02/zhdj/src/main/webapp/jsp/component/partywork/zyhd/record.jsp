...
<jsp:include page="../../../common/dqdp_vars.jsp">
    ...
    <jsp:param name="permission" value="volunteerRecord"></jsp:param>
</jsp:include>
<head>
...
<form action="" method="post" id="id_form_search">
    ...
    <table class="tableCommon mt5" width="100%" border="0" cellspacing="0" cellpadding="0" dataSrc="${baseURL}/volunteer/volunteerAction!ajaxView.action?id=${param.id}">
        <tbody>
        <tr>
            <td class="tdBlue">标题<input type="hidden" name="tbVolunteerActivityPO.id" id="volunteerActivityId"/></td>
            <td colspan="3" name="tbVolunteerActivityPO.title">
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <td class="tdBlue">志愿活动记录</td>
            <td colspan="3">
               <textarea rows="6" cols="80" name="tbVolunteerActivityPO.record" id="record" ></textarea>
            </td>
        </tr>
        </tbody>
    </table>
...
<div class="toolBar">
    <div align="center">
    	<input class="greenbtn mr10" type="button" id="save" permission="volunteerRecord" value="完成"/>&nbsp;
...
<script type="text/javascript">
 var userMap = new HashMap();
  $(document).ready(function(){
	  ...
	  KindEditor.ready(function(K) {
			editor1 = K.create('textarea[name="tbVolunteerActivityPO.record"]', {
			...
			afterCreate : function() {
	            ...
	            this.sync(); 
	            $('#id_form_search').find('textarea[name="tbVolunteerActivityPO.record"]').val(K('textarea[name="tbVolunteerActivityPO.record"]').val());
	        }  
		})});
  });
 ...
 $('#save').click(function(){ 
    ...
    if (dqdp.submitCheck("id_form_search")) {
        ...
        // 提交数据
        $('#id_form_search').attr("action","${baseURL}/volunteer/volunteerAction!ajaxSaveRecord.action");
    ...
    function initUserInfo(){
    	...
    	var qxNum = 0;
	    $.ajax({
	        url: "${baseURL}/meetingUser/meetingUserAction!ajaxVolunteerUserDetails.action",
	        data: {"id": $("#volunteerActivityId").val()},
	        type: "get",
	        dataType: "json",
	        success: function (result) {
	            if ("0" == result.code) {
