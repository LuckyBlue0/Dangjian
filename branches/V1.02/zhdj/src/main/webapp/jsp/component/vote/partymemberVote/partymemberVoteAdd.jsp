...
<form action="${baseURL}/partymembervote/partymembervoteAction!ajaxAdd.action" method="post" id="id_form_search">
    <div class="searchWrap">
            <div class="title">
                <h2 class="icon1">新增优秀党员评议</h2>
            </div>
        </div>
    ...
    <table class="tableCommon mt5" width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tbody>
        <tr>
            <td class="tdBlue" style="border: "><span style="color: red">*</span>评议主题</td>
            <td  >
    ...
    if (dqdp.submitCheck("id_form_search")) {
        ...
        $('#id_form_search').ajaxSubmit({
            ...
            success:function(result) {
                if ("0"==result.code) {
                    window.location.href="${baseURL}/partymembervote/partymembervoteAction!ajaxTurnToAddMember.action?id="+result.data.id+"&voteLimit="+result.data.voteLimit; 
   ...
   function uploadFile(){
	   $('#id_form_search').attr("action","${baseURL}/orgManage/orgManageAction!fileUpload.action");
	    $('#id_form_search').ajaxSubmit({
	     ...
	     $('#id_form_search').attr("action","${baseURL}/partymembervote/partymembervoteAction!ajaxAdd.action");
   }
