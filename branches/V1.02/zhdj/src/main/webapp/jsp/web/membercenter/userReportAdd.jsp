...
<div id="main">
             ...
             <form action="${baseURL}/membercenter/membercenterAction!addUserReport.action" method="post" id="id_form_add">
                    ...
                    <div class="act_tabWrap mt10">
                        <div class="pt20 pl20 pb100">
                            <dl class="clearfix">
                                ...
                                <dd class="ml5em"><input id="title" type="text" class="reginput2" placeholder="输入标题" name="tbIdeologyReportPO.title" valid="{must:true, length:200,fieldType:'pattern', tip:'标题'}">
                                <br/><font id="titleMsg" color="red"></font> </dd>
                            </dl>
                            <dl class="clearfix mt10">
                                <dt class="fl w4em tr">正文</dt>
                                <dd class="ml5em">
                                    <textarea name="tbIdeologyReportPO.content" onkeyup="countLength();" id="reportContent" placeholder="输入正文，不少于500字"  class="textarea2"></textarea>
                                <span id="msg">0</span>/500
                                <br/><font id="contentMsg" color="red"></font>
                                </dd>
                            </dl>
...
function countLength(){
	$("#msg").text($("#reportContent").val().length);
}

function doSave(type){
	if($("#title").val() == ''){
		$("#titleMsg").text("(标题不能为空)");
		$("#titleMsg").show();
		return;
	}else{
		$("#titleMsg").hide();
	...
	if(type == 0 && $("#reportContent").val().length < 500){
		$("#contentMsg").text("(内容不得少于500字)");
		$("#contentMsg").show();
		return;
	}else{
		$("#contentMsg").hide();
