...
@interface WishXinYuListVC ()

@end

@implementation WishXinYuListVC
...
- (void)viewDidLoad
{
    ...
    [self showBack];
    [self showNavBtn:72 target:self action:@selector(gotoWriteExperiencePage) title:@"写心语"];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refeshTableList) name:@"submitWishXinYu" object:nil];
}
...
{
    [self performSegueWithIdentifier:@"WriteWishVCID" sender:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)aTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    static NSString *identifier = @"wishListID";
    ...
    if (1) {
        ...
        labelSize = [contentLabel.text sizeWithFont:FONT(12) constrainedToSize:CGSizeMake(280, 999) lineBreakMode:UILineBreakModeWordWrap];
...
- (void)creatListModel
{
    ...
    [aDic setObject:[[NSString stringWithFormat:@"%@1%i10", self.idString, self.tableView.currentPage] MD5] forKey:@"digest"];
...
{
    if ([segue.identifier isEqualToString:@"WriteWishVCID"]) {
        WriteWishVC *vc = (WriteWishVC *)segue.destinationViewController;
