...
public abstract class ViewPageControll{
	...
	public ViewPageControll(Activity activity, AbsListView listView,
		...
		Log.e("--------------------eugene6  mRequestListenerAdapter--------------------"+mRequestListenerAdapter);
...
//				((TextView)mFooterView).setText("您还没有添加任何数据哦，赶紧去添加吧");
				...
				((BaseListActivity)mActivity).nullText.setVisibility(View.VISIBLE);
			}else{
				((TextView)mFooterView).setText("正在加载...");
				((BaseListActivity)mActivity).nullText.setVisibility(View.GONE);
			}
		...
		public void onExecuteFail(ResultObject resultObject) {
			completed();
			ViewPageControll.this.onExecuteFail(resultObject,mEmptyView, mPage);
		}
	...
	final public void loadPage(Map<String, Object> params,String method) {
		...
		if(needReflesh){
//			String mpage=SecurityUtil.encryptToDES(Constants.MICHI, mPage+"");
//			String mpagecount=SecurityUtil.encryptToDES(Constants.MICHI, mPageCount+"");
...
//		String keys=NoticeListActivity.parenttype+NoticeListActivity.type+mPage+mPageCount;
//		String digest=SecurityUtil.encryptToMD5(keys.toLowerCase());
//		params.put("digest", digest);
//		System.out.println("paramsString : " + StringUtil.toJsonString(params));
//		Map<String, String> map2 = new HashMap<String, String>();
//		map2=StringUtil.jiami(params);
		
		
//		map2.put("requestJson", StringUtil.toJsonString(params));

//		String key = Entryption.getJsonKey(params);
//		Log.e("请求key：" + key);
//		Log.e("请求接口地址：" + Constants.SERVER_URL + method);
//		mAQuery.ajax(Constants.SERVER_URL + method,key,String.class,mAjaxCallBack);
		System.out.println("url: " + Constants.SERVER_RUL2+method);
		mAQuery.ajax(Constants.SERVER_RUL2+method, StringUtil.jiami(params), String.class, mAjaxCallBack);
	}
