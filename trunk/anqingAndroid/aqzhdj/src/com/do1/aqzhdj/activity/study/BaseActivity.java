...
/**
 * 实现请求的父类 auth:YanFangqin data:2013-4-28 thzhd
 */
public class BaseActivity extends Activity implements IRequest, OnClickListener {
	 ...
	 *            头部右边按钮监听器
	 */
	public void setHeadView(View headView, int leftDrawable, String leftText,
			String centerTitle, int rightDrawable, String rightText,
			OnClickListener leftListener, OnClickListener rightListener) {
		if (headView != null) {

			if (leftDrawable != 0) {
				headView.findViewById(R.id.leftImage).setVisibility(
						View.VISIBLE);
				((TextView) headView.findViewById(R.id.leftImage))
						.setText(leftText);
				headView.findViewById(R.id.leftImage).setBackgroundResource(
						leftDrawable);
				headView.findViewById(R.id.leftImage).setOnClickListener(
						leftListener == null ? this : leftListener);
			}
			if (rightDrawable != 0) {
				...
				((TextView) headView.findViewById(R.id.rightImage))
						.setText(rightText);
				headView.findViewById(R.id.rightImage).setBackgroundResource(
						rightDrawable);
				headView.findViewById(R.id.rightImage).setOnClickListener(
						leftListener == null ? this : leftListener);
			}
			if (rightDrawable == R.drawable.btn_head_5
					&& centerTitle.length() > 6) {
				centerTitle = centerTitle.substring(0, 5) + "...";
			} else if (rightDrawable == R.drawable.btn_head_4
					&& centerTitle.length() > 7) {
	...
	public String toTime(String returnTime) {
		try {
			return constants.sdfTime.format(constants.sdfReturn
					.parse(returnTime));
	...
	public String toDate(String returnTime) {
		try {
			return constants.sdfDate.format(constants.sdfReturn
					.parse(returnTime));
		} catch (ParseException e) {
			e.printStackTrace();
		}
	...
	public String timeToDate(String returnTime) {
		try {
			return constants.sdfDate
					.format(constants.sdfTime.parse(returnTime));
		} catch (ParseException e) {
			e.printStackTrace();
		}
		return null;
