...
#import "DOScrollRefreshView.h"

@interface DOScrollRefreshView (PrivateMethods)
- (void) showCurrentPage;
@end

@implementation DOScrollRefreshView
...
-(void)resetCurrentPageHeight:(CGFloat)aFloat{
    if (aFloat<self.frame.size.height) {
        return;
    }
    //NSLog(@"resetCurrentPageHeight");
    _currentPageHeight = aFloat;
    self.contentSize= CGSizeMake(self.contentSize.width, _currentPageHeight);
    if (footerView) {
        footerView.frame = CGRectMake(0, _currentPageHeight, footerView.frame.size.width, footerView.frame.size.height);
    }
}

// Setup for init method with explicit values
- (id) initWithFrame:(CGRect)frame headerView:(UIView*)theHeaderView footerView:(UIView*)theFooterView startingAt:(NSUInteger)pageIndex delegate:(id<DOScrollRefreshViewDelegate,UIScrollViewDelegate>)verticalSwipeDelegate
{
    self = [super initWithFrame:frame];
    if (self)
    {
        ...
        _currentPageHeight = self.frame.size.height;
        self.contentSize= CGSizeMake(self.contentSize.width, _currentPageHeight);
    }
    return self;
}
...
-(void) setDelegate:(id<DOScrollRefreshViewDelegate,UIScrollViewDelegate>)newDelegate
{
    if (newDelegate != (id<DOScrollRefreshViewDelegate,UIScrollViewDelegate>)self)
...
//        if (scrollView.contentOffset.y > footerView.frame.size.height)
       // NSLog(@"--%f---%f",scrollView.contentOffset.y,footerView.frame.size.height+_currentPageHeight-scrollView.frame.size.height);
        
        if (scrollView.contentOffset.y > footerView.frame.size.height+_currentPageHeight-scrollView.frame.size.height)
            //
        {
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
    ...
    if (_headerLoaded)
    {
        ...
        previousPage.frame = self.bounds;
    ...
    {
        if ([externalDelegate pageCount]==0) {
            return;
        }
    ...
    //NSLog(@"==%f,%f,%f,%f",self.currentPageView.frame.origin.x,self.currentPageView.frame.origin.y,self.currentPageView.frame.size.width,self.currentPageView.frame.size.height);
}
...
- (void)pageAnimationDidStop:(NSString *)animationID finished:(NSNumber *)finished context:(void *)context
{
    // Remove the old page
    [currentPageView removeFromSuperview];
    //self.currentPageView.frame = CGRectMake(0, 0, self.currentPageView.frame.size.width, self.currentPageView.frame.size.height);
    // Set the previous/next page we just animated into view as the current page
    UIView* newPage = (UIView*)context;
    newPage.frame = CGRectMake(0, 0, newPage.frame.size.width, newPage.frame.size.height);
