...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		_zoomEnabled = YES;
		_rotateEnabled = YES;
		_orientation = UIInterfaceOrientationPortrait;//**** kllmctrl
		...
		_centerPageAnimationDuration = 0.3;//**** kllmctrl
...
- (void)dealloc {
	...
	DO_RELEASE_SAFELY(_animationStartTime);
	DO_RELEASE_SAFELY(_pages);
	DO_RELEASE_SAFELY(_pageQueue);
	
	DO_RELEASE_SAFELY(_touch1);
	DO_RELEASE_SAFELY(_touch2);
...
- (CGRect)frameOfPageAtIndex:(NSInteger)pageIndex {
	...
	if (UIInterfaceOrientationIsLandscape(_orientation)) {
		if (size.width / size.height > self.frame.size.width / self.frame.size.height) {
			...
			width = size.height/size.width * self.frame.size.height;
...
- (void)layoutPage {
	...
	// Layout.
	if (nil != page) {
		//CGAffineTransform rotation=CGAffineTransformMakeRotation(M_PI);// = TTRotateTransformForOrientation(_orientation);
...
- (void)setDataSource:(id<DOScrollViewDataSource>)dataSource {
