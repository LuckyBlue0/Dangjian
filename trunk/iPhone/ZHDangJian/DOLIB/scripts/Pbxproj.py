"""
    ...
    modify content:
    1.modify: Header search paths:.....
    def add_header_search_path(self, configuration):
		did_add_build_setting = self.add_build_setting(configuration,	'HEADER_SEARCH_PATHS',	'"../DOLIB/DO/**"')
		return did_add_build_setting
    
    2.add depend projcrt to dic "Common"
    def add_file_to_Common(self, name, guid):
		return self.add_file_to_group(name, guid, 'Common')
...
class Pbxproj(object):
	...
	# Add a file to the Common PBXGroup.
	#
	# <guid> /* <name> */,
	def add_file_to_Common(self, name, guid):
		return self.add_file_to_group(name, guid, 'Common')
	...
	def add_dependency(self, dep):
		...
		self.set_project_data(project_data)
#        add_file_to_Common
		if not self.add_file_to_Common(dep._project_name+".xcodeproj", pbxfileref_hash):
