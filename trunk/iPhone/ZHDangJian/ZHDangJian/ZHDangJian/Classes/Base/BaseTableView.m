...
-(id)initWithCoder:(NSCoder *)aDecoder{
    if (self=[super initWithCoder:aDecoder]) {
        ...
        BaseHeaderView *aHeaderView = [[[BaseHeaderView alloc] initWithFrame:CGRectMake(0.0f, -self.height, self.width, self.height) isHeader:YES] autorelease];
        ...
        BaseTableViewFooterView *aFooter = [[[BaseTableViewFooterView alloc] initWithFrame:
                                             CGRectMake(0, 0, self.width, H_FOOTER)] autorelease];
        ...
        errorView = [[DOErrorView alloc] initWithTitle:NetWorkFaild  subtitle:@"" image:nil];
        errorView.frame = self.bounds;
        errorView.backgroundColor = WHITE_COLOR;
...
-(void)dealloc{
    ...
    DO_RELEASE_SAFELY(headerView);
    
    DO_RELEASE_SAFELY(footerView);
    
    baseDelegate = nil;
    
    DO_RELEASE_SAFELY(datas);
    
    DO_RELEASE_SAFELY(loadingView);
    DO_RELEASE_SAFELY(emptyView);
    DO_RELEASE_SAFELY(errorView);
...
-(void)finishLoadingDataAction:(NSArray*)aArray{
    ...
    if (aArray) {
        for (int i=0; i<[aArray count]; i++) {
            [datas addObject:[aArray objectAtIndex:i]];
        }
    }
