...
#import "BranchActivityListVC.h"
#import "BranchActivityStatueVC.h"

@interface BranchActivityListVC ()

@end

@implementation BranchActivityListVC
...
- (void)viewDidLoad
{
    ...
    [self creatBranchListModel];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refeshTableList) name:@"branchLeaveSuccess" object:nil];
}
...
- (void)dealloc {
    ...
    [_tableView release];
    _branchListModel.delegate = nil;
    DO_RELEASE_SAFELY(_branchListModel);
...
- (UITableViewCell *)tableView:(UITableView *)aTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    static NSString *identifier = @"branchActivityListCellID";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self performSegueWithIdentifier:@"BranchActivityStatueVCID" sender:nil];
}
...
- (void)creatBranchListModel
{
    if (_branchListModel) {
        _branchListModel.delegate = nil;
        DO_RELEASE_SAFELY(_branchListModel);
    }
    _branchListModel = [[BaseModel alloc] init];
    _branchListModel.delegate = self;
    ...
    NSString *requestUrl = [NSString stringWithFormat:KBranchActiveListUrlFormat, KURL, [BaseVC buildJson:aDic]];
    [_branchListModel startRequest:requestUrl];
}
        ...
        if (aRR.code) {
            if (aRR.dataDic){
                ...
                if ([[aRR.dataDic objectForKey:@"toDoCount"] intValue] > 0) {
                    self.title = [NSString stringWithFormat:@"支部活动（%i）", [[aRR.dataDic objectForKey:@"toDoCount"] intValue]];
                } else {
                    self.title = @"支部活动";
                }
...
{
    if ([segue.identifier isEqualToString:@"BranchActivityStatueVCID"]) {
        BranchActivityStatueVC *vc = (BranchActivityStatueVC *)segue.destinationViewController;
