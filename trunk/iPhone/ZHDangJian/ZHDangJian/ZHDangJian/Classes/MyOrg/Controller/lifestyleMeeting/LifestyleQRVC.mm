...
#import "LifestyleQRVC.h"
#import <QRCodeReader.h>
#import "LifestyleResultVC.h"

@interface LifestyleQRVC ()

@end

@implementation LifestyleQRVC
...
- (void)zxingController:(ZXingWidgetController*)controller didScanResult:(NSString *)result {
    ...
    if (result) {
        if ([result rangeOfString:@";"].length > 0) {
            ...
            [self performSegueWithIdentifier:@"LifestyleQRVCID" sender:self];
        } else {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if ([segue.identifier isEqualToString:@"LifestyleQRVCID"]) {
        LifestyleResultVC *vc = (LifestyleResultVC *)segue.destinationViewController;
