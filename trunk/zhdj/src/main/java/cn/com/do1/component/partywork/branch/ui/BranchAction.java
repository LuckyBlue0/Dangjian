...
public class BranchAction extends BaseAction {
	private final static transient Logger logger = LoggerFactory.getLogger(BranchAction.class);
	@Resource
	...
	public void ajaxSave() throws Exception, BaseException {
		// todo:完成新增的代码;
		Map<String, Object> result = branchService.saveBranchActivityInfo(tbBranchActivityPO, userIds);
	...
	public void ajaxSaveRecord() throws Exception, BaseException{
		Map<String, Object> result = branchService.saveBranchRecordInfo(tbBranchActivityPO, userIds);
		setActionResult(result.get("code").toString(), result.get("desc").toString());
		doJsonOut();
	}
