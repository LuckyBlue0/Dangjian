...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<jsp:include page="../../../common/dqdp_vars.jsp">
		...
		<jsp:param name="permission"
			value="musicFileManage,musicFileDel,musicFileEdit,musicFileAdd"></jsp:param>
	...
	<body style="background: #f6ebe5;">
		...
		<!-- 新闻类型信息搜索表单 -->
		<form action="${baseURL}/musicfile/musicfileAction!ajaxSearch.action"
									...
									<td>
										<table class="search" width="100%" border="0" cellspacing="0"
											...
											<tr>
												...
												<td width="140">
													<select  name="searchValue.type" dictType="musicType"></select>
												</td>
												<td>
													<input class="btnQuery" name="search" type="button" value="查询" permission="" onclick="javascript:doSearch(1);" />
													<input class="btnQuery" type="button" value="新增" permission="musicFileAdd" onclick="javascript:doVideoFileAdd()"/>&nbsp;
		                                            <input class="btnQuery" type="button" value="删除" permission="musicFileDel" onclick="javascript:_doDel('userInfo_list');doSearch(1);" />
												</td>
											</tr>
			...
			delUrl="${baseURL}/musicfile/musicfileAction!ajaxBatchDelete.action"></div>
		<div id="tip"></div>
...
$(document).ready(function() {
	doSearch(1);
	 ...
	 $.ajax({
            url:'${baseURL}/musicfile/musicfileAction!uploadFile.action',
            data:{"tbMusicfilePO.filePath":$("#filePath").val(),"tbMusicfilePO.fileName":$("#fileName").val()},  
            type:'post',                                     
            dataType:'json',
											...
											operations : [
													{
														...
														event : function(index,
																content) {
															window.location.href = 'musicFileView.jsp?id='+ content.id;
														}
													},
													{
														name : "修改",
														permission : "musicFileEdit",
														event : function(index, 
																content) {
															window.location.href = 'musicFileEdit.jsp?id='+ content.id;
																
														}
													},
													{
														name : "删除",
														permission : "musicFileDel",
														event : function(index,
																content) {
															_doSignlDel(
																	"userInfo_list",
...
function doVideoFileAdd(){
	window.location.href="${baseURL}/jsp/component/data/musicFile/musicFileAdd.jsp"; 
}
</script>
