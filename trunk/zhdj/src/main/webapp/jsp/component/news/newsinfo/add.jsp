...
<jsp:include page="../../../common/dqdp_vars.jsp">
    ...
    <jsp:param name="permission" value="newsManager,newsAdd"></jsp:param>
</jsp:include>
<head>
...
<form action="${baseURL}/news/newsinfo/newsInfoAction!ajaxSave.action" method="post" id="id_form_add">
      <div class="searchWrap">
            <div class="title">
                <h2 class="icon1">${name}信息录入</h2>
	           	<input type="hidden" id="newsInfoId" name="tbNewsInfoPO.newsInfoId"/>
	        ...
	        <c:forEach items="${map}" var="o">
	        	<c:if test="${o.key == 0}">
			        <tr>
			            ...
			            <td >
			                <input type="text" id="title" name="tbNewsInfoPO.title" valid="{must:true,tip:'${o.value}',fieldType:'pattern'}"/>&nbsp;&nbsp;&nbsp;
			            </td>
			        </tr>
		        </c:if>
		        <c:if test="${o.key == 1}">
		        <tr>
		            ...
		            <td>
		            	<select dictType="newsProjectType" name="tbNewsInfoPO.projectType" id="projectType" valid="{must:true,tip:'${o.value}',fieldType:'pattern'}"/>
		            </td>
		        </tr>
		        </c:if>
		        <c:if test="${o.key == 2}">
		        <tr>
		            <td width="120" class="tdBlue"><span style="color: red">*</span>&nbsp;${o.value}</td>
		            <td>
		            	<img src="${baseURL}/jsp/web/images/default.jpg" id="pic" height="150" width="200" alt="" />
						<input type="hidden" id="path" readonly="readonly" placeholder="图片建议尺寸：360像素 * 200像素" name="tbNewsInfoPO.imgPath" valid="{must:true,tip:'${o.value}',fieldType:'pattern'}"/>
		        ...
		        <tr>
		            <td class="tdBlue"><span style="color: red">*</span>&nbsp;${o.value}</td>
		            <td colspan="3">
		               <input type="text" name="tbNewsInfoPO.tabBar" id="tabBar" valid="{must:true,tip:'${o.value}',fieldType:'pattern'}"/>
		            </td>
		        </tr>
		        </c:if>
		        ...
		        <tr>
		            <td class="tdBlue"><span style="color: red">*</span>&nbsp;${o.value}</td>
		            <td colspan="3">
		                <input type="text" name="tbNewsInfoPO.source" id="source" valid="{must:true,tip:'${o.value}',fieldType:'pattern'}"/>
		            </td>
		        </tr>
		        </c:if>
		        <c:if test="${o.key == 5}">
		        <tr>
		            <td class="tdBlue"><span style="color: red">*</span>&nbsp;${o.value}</td>
		            <td colspan="3">
		              <textarea rows="3" cols="80" name="tbNewsInfoPO.bodyDigest" id="bodyDigest" valid="{must:true,tip:'${o.value}',fieldType:'pattern'}"></textarea>
		            </td>
		        </tr>
		        </c:if>
		        <c:if test="${o.key == 6}">
		        <tr>
		            <td class="tdBlue"><span style="color: red">*</span>&nbsp;${o.value}</td>
		            <td colspan="3">
		               <textarea rows="6" cols="80" name="tbNewsInfoPO.content" id="content" valid="{must:true,tip:'${o.value}',fieldType:'pattern'}"></textarea>
		            </td>
		        </tr>
		        </c:if>
	        </c:forEach>
	...
	<div class="toolBar">
    <div align="center">
    	<input class="greenbtn mr10" type="button" id="save" permission="newsAdd" value="保存"/>&nbsp;
