...
<jsp:include page="../../../common/dqdp_vars.jsp">
    <jsp:param name="dict" value="newsInfoStatus,meetingWhetherEnd,meetingWhetherRecommend"></jsp:param>
    <jsp:param name="permission" value="branchManager,branchSearch,branchView,branchAdd,branchEdit,branchDel,branchRecommended,branchRecord,branchPush,branchOut"></jsp:param>
...
<!--公告 end-->
<form action="${baseURL}/branch/branchAction!ajaxSearch.action" method="post" id="form_search">
<div class="searchWrap">
    <div class="title clearfix">
        <h2 class="icon1">支部活动</h2>
    </div>
</div>
...
<!-- 新闻类型信息搜索表单 -->
                            ...
                            <td>
                                <table class="search" width="100%" border="0" cellspacing="0" cellpadding="0">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                        	<input class="btnQuery" name="search" type="button" value="查询" permission="branchSearch" onclick="javascript:doSearch(1);"/>
                                        	<input class="btnQuery" type="button" permission="branchAdd" value="新增" onclick="javascript:window.location.href='add.jsp'+ '?dqdp_csrf_token='+dqdp_csrf_token" />
       										<input class="btnQuery" type="button" permission="branchDel" value="删除" onclick="javascript:_doDel('branchInfo_list');doSearch(1);"/>
										</td>
                                    </tr>
	...
	<div id="branchInfo_list" delUrl="${baseURL}/branch/branchAction!ajaxBatchDelete.action"></div>
	<div id="tip"></div>
	<div class="toolBar">
...
<script type="text/javascript">
                    ...
                    var list1 = new ListTable(
                            {
                                ...
                                operations:[
                                    {name:"查看", permission:"branchView", event:function (index, content) {
                                        window.location.href = 'view.jsp?id=' + content.id+ '&dqdp_csrf_token='+dqdp_csrf_token;
                                    }},
                                    {name:"二维码", permission:"branchView", event:function (index, content) {
                                        window.location.href = '${baseURL}/jsp/component/partywork/shyk/meeting/recode.jsp?id=' + content.id+ '&type=3';
                                    }},
                                    {name:"会议记录", permission:"branchRecord", event:function (index, content) {
                                         window.location.href="record.jsp?id="+ content.id+ '&dqdp_csrf_token='+dqdp_csrf_token;
                                    }},
                                    {name:"总结发布", permission:"branchView", condition:function (index, content) {
											return content.whetherEnd == 1;
											return true;
											},event:function (index, content) {
                                        window.location.href = '${baseURL}/jsp/component/news/tissuemien/edit.jsp?id=' + content.id+ '&type=2&backURL=${baseURL}/jsp/component/partywork/zbhd/list.jsp';
                                    }},
                                    {name:"编辑", permission:"branchEdit", condition:function (index, content) {
											return content.status == 0;
											}, event:function (index, content) {
                                    	window.location.href = 'edit.jsp?id=' + content.id+ '&dqdp_csrf_token='+dqdp_csrf_token;
                                    }}, 
                                    {name:"发布", permission:"branchPush", condition:function (index, content) {
											return content.status == 0;
											}, event:function (index, content) {
                                    	pushOrOut(content.id,1);
                                    }}, 
                                    {name:"下架", permission:"branchOut", condition:function (index, content) {
											return content.status == 1;
											}, event:function (index, content) {
                                    	pushOrOut(content.id,2);
                                    }}, 
	                                {name:"删除", permission:"branchDel", condition:function (index, content) {
											return content.status != 1;
											},event:function (index, content) {
	                                    _doSignlDel("branchInfo_list", content.id);
	                                }}
                                    
                                ],
                                trStyle:["trWhite"]
                            });
                    list1.createList("branchInfo_list");
 ...
 function pushOrOut(id,type){
	...
	if(confirm(alertDesc)){
	    $.ajax({
	        url: "${baseURL}/branch/branchAction!pushOrOut.action",
