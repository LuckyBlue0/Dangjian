...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<jsp:include page="../../../common/dqdp_vars.jsp">
		...
		<jsp:param name="permission"
			value="orgVoteAdd,orgVoteEdit,orgVoteDel,orgVotePushOrOut"></jsp:param>
	...
	<body style="background: #f6ebe5;">
        ...
        <div class="searchWrap">
            <div class="title clearfix">
                <h2 class="icon1">民主机关评议</h2>
            </div>
        </div>
		

		<!-- 新闻类型信息搜索表单 -->
		<form action="${baseURL}/orgvote/orgvoteAction!ajaxSearch.action"
									...
									<td>
										<table class="search" width="100%" border="0" cellspacing="0"
											...
											<tr>
												...
												<td width="140">
													<input name="searchValue.topic" />
												</td>
												<td>
													<input class="btnQuery" name="search" type="button" value="查询"  onclick="javascript:doSearch(1);" />
													<input class="btnQuery" type="button" value="新增" permission="orgVoteAdd" onclick="window.location.href='orgVoteAdd.jsp'" />
		                                            <input class="btnQuery" type="button" value="删除" permission="orgVoteDel" onclick="javascript:_doDel('userInfo_list');doSearch(1);" />
												</td>
											</tr>
			...
			delUrl="${baseURL}/orgvote/orgvoteAction!ajaxBatchDelete.action"></div>
		<div id="tip"></div>
...
function doSearch($pageIndex) {
											...
											operations : [
													{
														...
														event : function(index,
																content) {
															window.location.href = '${baseURL}/orgvote/orgvoteAction!view.action?id='+ content.id;
														}
													},
													{
														name : "编辑",
														permission : "orgVoteEdit",
														condition : function(
																index, content) {
														...
														event : function(index,
																content) {
															window.location.href = '${baseURL}/orgvote/orgvoteAction!toEdit.action?id='+ content.id;
														}
													},
													{
														name : "发布",
														permission : "orgVotePushOrOut",
														condition : function(
																index, content) {
	...
	function pushOrOut(id,type){
		...
		if(confirm(alertDesc)){
		    $.ajax({
		        url: "${baseURL}/orgvote/orgvoteAction!pushOrOut.action",
		        data: {"tbOrgVotePO.id": id,"tbOrgVotePO.pushStatus":type},
		        type: "post",
		        dataType: "json",
		        success: function (result) {
		            if ("0" == result.code) {
